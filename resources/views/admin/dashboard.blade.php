...
<div class="page-header">
  <div>
    ...
    <div class="page-sub">Welcome back, {{ auth()->user()->name }}! Here's what's happening today.</div>
  </div>
  <div class="page-actions">
    <a href="{{ route('admin.reports.sales') }}" class="btn btn-secondary">📈 View Reports</a>
...
<div class="stat-grid">
  <div class="stat-card">
    ...
    <div class="stat-foot">vs ₹{{ number_format($lastMonthRevenue ?? 0) }} last month</div>
  </div>
  ...
  <div class="stat-card">
    ...
    <div class="stat-foot">{{ $todayOrders ?? 0 }} orders today</div>
  </div>
  ...
  <div class="stat-card">
    ...
    <div class="stat-foot">{{ $newCustomers ?? 0 }} new this month</div>
  </div>
  ...
  <div class="stat-card">
    ...
    <div class="stat-foot">{{ $lowStockCount ?? 0 }} low stock alerts</div>
  </div>
</div>

{{-- QUICK LINKS --}}
<div class="quick-grid">
  <a href="{{ route('admin.orders.index') }}?status=pending" class="quick-card">
    <div class="quick-icon" style="background:#fef9c3">⏳</div>
    <div>
      <div class="quick-num">{{ $pendingOrders ?? 0 }}</div>
      <div class="quick-lbl">Pending Orders</div>
      <div class="quick-change up">Action needed</div>
    </div>
  </a>
  <a href="{{ route('admin.inventory.low') }}" class="quick-card">
    <div class="quick-icon" style="background:#fef2f2">⚠️</div>
    <div>
      <div class="quick-num">{{ $lowStockCount ?? 0 }}</div>
      <div class="quick-lbl">Low Stock Items</div>
      <div class="quick-change down">Needs restock</div>
    </div>
  </a>
  <a href="{{ route('admin.reviews.index') }}?approved=0" class="quick-card">
    <div class="quick-icon" style="background:#f5f3ff">⭐</div>
    <div>
      <div class="quick-num">{{ $pendingReviews ?? 0 }}</div>
    ...
    <div class="card-body">
      ...
      <div style="margin-top:16px;display:flex;flex-direction:column;gap:8px">
        <div style="display:flex;align-items:center;justify-content:space-between;font-size:13px">
          ...
          <div><strong>{{ $statusCounts['pending'] ?? 0 }}</strong></div>
        </div>
        <div style="display:flex;align-items:center;justify-content:space-between;font-size:13px">
          ...
          <div><strong>{{ $statusCounts['processing'] ?? 0 }}</strong></div>
        </div>
        <div style="display:flex;align-items:center;justify-content:space-between;font-size:13px">
          ...
          <div><strong>{{ $statusCounts['delivered'] ?? 0 }}</strong></div>
        </div>
        <div style="display:flex;align-items:center;justify-content:space-between;font-size:13px">
          ...
          <div><strong>{{ $statusCounts['cancelled'] ?? 0 }}</strong></div>
        </div>
      </div>
    </div>
  </div>
...
<div style="display:grid;grid-template-columns:1fr 340px;gap:16px;margin-bottom:24px" class="two-col">
    ...
    <div class="table-wrap" style="border:none;border-radius:0">
      <table>
        ...
        <tbody>
          ...
          <tr>
            <td><span style="font-weight:600;color:var(--green)">#{{ $order->order_number }}</span></td>
            <td>
              <div class="td-flex">
                ...
                <div>
                  <div style="font-size:13px;font-weight:500">{{ $order->user->name ?? 'N/A' }}</div>
            ...
            <td style="color:var(--text-muted);font-size:12px">{{ $order->created_at->format('d M, h:i A') }}</td>
            <td>
              <a href="{{ route('admin.orders.show', $order->id) }}" class="btn btn-secondary btn-sm btn-icon" title="View">👁</a>
            </td>
          </tr>
          @empty
  ...
  <div class="card">
    <div class="card-header">
      ...
      <a href="{{ route('admin.reports.products') }}" class="btn btn-secondary btn-sm">View All</a>
    </div>
    <div class="card-body">
      @forelse($topProducts ?? [] as $i => $product)
      <div class="top-product">
        <div class="tp-rank {{ $i === 0 ? 'gold' : '' }}">#{{ $i+1 }}</div>
        <div class="tp-img">{{ $product->category->image ?? '🛒' }}</div>
        <div style="flex:1;min-width:0">
          <div class="tp-name" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ $product->name }}</div>
          <div class="tp-sales">{{ $product->order_items_count ?? 0 }} sold</div>
        </div>
        <div class="tp-rev">₹{{ number_format($product->order_items_sum_subtotal ?? 0) }}</div>
      </div>
      @empty
...
new Chart(statusCtx, {
  ...
  data: {
    ...
    datasets: [{
      data: [
        {{ $statusCounts['pending'] ?? 12 }},
        {{ $statusCounts['processing'] ?? 8 }},
        {{ $statusCounts['delivered'] ?? 45 }},
        {{ $statusCounts['cancelled'] ?? 5 }}
      ],
