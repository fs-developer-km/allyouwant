...
<title>@yield('title', 'Dashboard') — GroceryMart Admin</title>
...
<aside class="sidebar" id="sidebar">
  ...
  <nav>
    ...
    <div class="nav-group">
      ...
      <div class="nav-item {{ request()->routeIs('admin.categories.*') ? 'active' : '' }}" onclick="toggleSub('cat-sub', this)">
        <span class="nav-icon">🗂️</span> Categories
        <span class="nav-chevron">▾</span>
      </div>
      <div class="nav-sub {{ request()->routeIs('admin.categories.*') ? 'open' : '' }}" id="cat-sub">
        <a href="{{ route('admin.categories.index') }}" class="{{ request()->routeIs('admin.categories.index') ? 'active' : '' }}">All Categories</a>
        <a href="{{ route('admin.categories.create') }}" class="{{ request()->routeIs('admin.categories.create') ? 'active' : '' }}">Add New</a>
      </div>

      <div class="nav-item {{ request()->routeIs('admin.products.*') ? 'active' : '' }}" onclick="toggleSub('prod-sub', this)">
        <span class="nav-icon">🛒</span> Products
        <span class="nav-chevron">▾</span>
      </div>
      <div class="nav-sub {{ request()->routeIs('admin.products.*') ? 'open' : '' }}" id="prod-sub">
        <a href="{{ route('admin.products.index') }}" class="{{ request()->routeIs('admin.products.index') ? 'active' : '' }}">All Products</a>
        <a href="{{ route('admin.products.create') }}" class="{{ request()->routeIs('admin.products.create') ? 'active' : '' }}">Add New</a>
      </div>

      <a href="{{ route('admin.inventory.index') }}" class="nav-item {{ request()->routeIs('admin.inventory.*') ? 'active' : '' }}">
        <span class="nav-icon">📦</span> Inventory
        @php try { $lowStockCount = \App\Models\Product::whereColumn('stock_quantity','<=','low_stock_alert')->count(); } catch(\Exception $e) { $lowStockCount = 0; } @endphp
        @if($lowStockCount > 0)
          <span class="nav-badge red">{{ $lowStockCount }}</span>
        @endif
      </a>
    </div>
    ...
    <div class="nav-group">
      ...
      <a href="{{ route('admin.orders.index') }}" class="nav-item {{ request()->routeIs('admin.orders.*') ? 'active' : '' }}">
        ...
        @php try { $pendingOrdersCount = \App\Models\Order::where('status','pending')->count(); } catch(\Exception $e) { $pendingOrdersCount = 0; } @endphp
  ...
  <div class="sidebar-bottom">
    <div class="admin-profile">
      <div class="admin-avatar">{{ substr(auth()->user()->name, 0, 2) }}</div>
      <div>
...
<header class="header">
  ...
  <div class="header-right">
    ...
    <div class="header-profile">
      <div class="h-avatar">{{ substr(auth()->user()->name, 0, 2) }}</div>
      <div>
