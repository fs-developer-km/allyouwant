...
<nav class="navbar-wrap">
  <div class="nav-inner">
    ...
    <form action="{{ route('shop.search') }}" method="GET" class="nav-search">
      <input type="text" name="q" placeholder="Search products, categories..." value="{{ request('q') }}">
      <button type="submit">🔍 Search</button>
    </form>
    <div class="nav-right">
      @auth
        <a href="{{ route('account.index') }}" class="nav-btn">👤 Account</a>
      @else
        <a href="{{ route('login') }}" class="nav-btn">Login</a>
      @endauth
      {{-- <a href="{{ route('wishlist.index') }}" class="nav-btn">🤍</a> --}}
      <a href="{{ route('cart.index') }}" class="nav-btn btn-cart-nav">
        🛒 Cart
  ...
  <div class="cats-hero-inner">
    <div class="cats-hero-text">
      ...
      <div class="cats-hero-sub">
        Explore {{ $categories->count() }}+ categories — from fresh vegetables to<br>
        dairy, bakery, beverages & more. Delivered in 60 min!
      </div>
      <div class="cats-hero-stats">
        <div><div class="hs-num">{{ $categories->count() }}+</div><div class="hs-lbl">Categories</div></div>
        <div><div class="hs-num">{{ $categories->sum('active_products_count') }}+</div><div class="hs-lbl">Products</div></div>
...
<div class="page-content">
  ...
  <div style="margin-bottom:6px">
    ...
    <div class="popular-row" id="popularRow">
      ...
      @php $icon = $icons[$cat->name] ?? '🛒'; @endphp
      <a href="{{ route('category.show', $cat->slug) }}" class="pop-item">
        <div class="pop-img">
          @if($cat->image)<img src="{{ asset('storage/'.$cat->image) }}" alt="{{ $cat->name }}">
          @else<span>{{ $icon }}</span>@endif
        </div>
        <div class="pop-name">{{ $cat->name }}</div>
        <div class="pop-cnt">{{ $cat->active_products_count }} items</div>
      </a>
      @endforeach
    </div>
  </div>
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:10px">
    <div style="font-size:13.5px;color:var(--muted)">
      Showing <strong id="catCount" style="color:var(--text)">{{ $categories->count() }}</strong> categories
    </div>
  ...
  <div class="cat-grid" id="catGrid">
    ...
    <a href="{{ route('category.show', $cat->slug) }}"
       ...
       data-count="{{ $cat->active_products_count }}">
      ...
      <div class="cat-card-body">
        ...
        <div class="cat-card-count">
          {{ $cat->active_products_count > 0 ? $cat->active_products_count.' products' : 'Coming soon' }}
        </div>
      </div>
      ...
      {{-- Accent line --}}
      <div class="cat-card-accent" style="background:{{ $accent }}"></div>
  ...
  <div class="offers-strip">
    <a href="{{ route('offers') }}" class="offer-strip-card osc1">
    ...
    <a href="{{ route('register') }}" class="offer-strip-card osc2">
      <div class="osc-glow"></div>
      <div class="osc-emoji">🎁</div>
      <div class="osc-text">
    ...
    <a href="{{ route('shop') }}" class="offer-strip-card osc3">
      <div class="osc-glow"></div>
      <div class="osc-emoji">⚡</div>
      <div class="osc-text">
  ...
  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-top:40px" class="feat-bottom-grid">
    @foreach([['🚚','Free Delivery','On orders above ₹499'],['⚡','60-Min Express','Order & get in 60 min'],['🌿','100% Fresh','Farm to doorstep daily'],['🔒','Secure Pay','UPI, Cards, COD']] as $feat)
    <div style="background:var(--white);border:1.5px solid var(--border);border-radius:12px;padding:18px 16px;display:flex;align-items:center;gap:13px;transition:all .2s" class="feat-bottom-item">
      <div style="width:42px;height:42px;border-radius:10px;background:var(--gl);display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">{{ $feat[0] }}</div>
      <div>
        <div style="font-size:13.5px;font-weight:700;color:var(--text)">{{ $feat[1] }}</div>
        <div style="font-size:12px;color:var(--muted);margin-top:1px">{{ $feat[2] }}</div>
      </div>
    </div>
    @endforeach
  </div>
...
<div style="background:#0f1c14;color:rgba(255,255,255,.5);text-align:center;padding:22px;font-size:13px">
  © {{ date('Y') }} All You Want Grocery ·
  <a href="{{ route('home') }}" style="color:#4ade80">Home</a> ·
  <a href="{{ route('shop') }}" style="color:#4ade80">Shop</a> ·
  <a href="{{ route('offers') }}" style="color:#4ade80">Offers</a>
</div>

{{-- ── MOBILE BOTTOM NAV ── --}}
<div class="mob-bottom-nav">
  <div class="mbn-inner">
    <a href="{{ route('home') }}" class="mbn-item">
    ...
    <a href="{{ route('shop') }}" class="mbn-item">
    ...
    <a href="{{ route('categories.all') }}" class="mbn-item active">
    ...
    <a href="{{ route('cart.index') }}" class="mbn-item" style="position:relative">
      <svg viewBox="0 0 24 24"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
      Cart
      @if($cc > 0)<span class="mob-bottom-badge" style="position:absolute;top:4px;right:8px;background:#e53935;color:#fff;font-size:9px;font-weight:700;min-width:15px;height:15px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:2px solid #fff">{{ $cc }}</span>@endif
    </a>
    <a href="{{ auth()->check() ? route('account.index') : route('login') }}" class="mbn-item">
