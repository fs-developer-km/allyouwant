  ...
  <div class="navbar-inner">
    ...
    <div class="search-wrap">
      <form action="{{ route('shop.search') }}" method="GET" class="search-box">
        ...
        <input class="search-input" type="text" name="q" placeholder="Search for milk, vegetables, fruits..." value="{{ request('q') }}">
...
<div class="cat-nav">
  <div class="cat-nav-inner">
    <a href="{{ route('home') }}" class="cat-link active">🏠 Home</a>
    ...
    @foreach($categories as $cat)
      @php
        ...
        $icon = $icons[$cat->name] ?? '🛒';
      @endphp
      <a href="{{ route('category.show', $cat->slug) }}" class="cat-link">{{ $icon }} {{ $cat->name }}</a>
    @endforeach
    <a href="{{ route('offers') }}" class="cat-link" style="color:#e53935;border-bottom-color:#e53935">🔥 Offers</a>
  </div>
</div>
  ...
  <div class="hero-section">
    <div class="hero-grid">
      ...
      <div class="sidebar-cats">
        ...
        <a href="{{ route('category.show', $cat->slug) }}" class="sidebar-cat-item">
          <div class="sidebar-cat-left">
            <div class="sidebar-cat-icon" style="background:{{ $bg }}">{{ $icon }}</div>
      ...
      <div class="hero-right">
        @php $vegCat = $categories->firstWhere('name','Vegetables'); $fruitCat2 = $categories->firstWhere('name','Fruits'); $dairyCat = $categories->firstWhere('name','Dairy & Eggs'); @endphp
    ...
    <div class="hero-right-mobile" style="display:none">
      <a href="{{ $fruitCat2 ? route('category.show',$fruitCat2->slug) : route('shop') }}" class="mini-card mini-c1"><div class="mini-emoji">🍓</div><div class="mini-tag">Fresh Picks</div><div class="mini-title">Seasonal Fruits</div><div class="mini-cta">Shop Now →</div></a>
      <a href="{{ $vegCat ? route('category.show',$vegCat->slug) : route('shop') }}" class="mini-card mini-c2"><div class="mini-emoji">🥦</div><div class="mini-tag">Farm Direct</div><div class="mini-title">Organic Veggies</div><div class="mini-cta">Shop Now →</div></a>
    </div>
  </div>
</div>
...
<div class="page-wrap">
  ...
  <div class="section">
    ...
    <div class="products-row">
      ...
      @php $icons = ['Vegetables'=>'🥬','Fruits'=>'🍎','Dairy & Eggs'=>'🥛','Meat & Fish'=>'🍗','Bakery'=>'🧁','Beverages'=>'🧃','Instant Food'=>'🍜','Personal Care'=>'🧴','Household'=>'🏠','Pet Care'=>'🐾']; $pIcon = $icons[$product->category->name ?? ''] ?? '🛒'; @endphp
      <div class="prod-card">
        @if($product->is_on_sale)<div class="prod-badge badge-sale">{{ $product->discount_percent }}% OFF</div>
        @elseif($product->is_new_arrival)<div class="prod-badge badge-new">NEW</div>
        @elseif($product->is_bestseller)<div class="prod-badge badge-hot">HOT</div>@endif
        ...
        <div class="prod-body">
          ...
          <div class="prod-footer">
            <div class="prod-price-wrap">
              ...
              @if($product->is_on_sale)<div class="prod-price-old">₹{{ number_format($product->price) }}</div>@endif
            </div>
  ...
  <div class="section">
    ...
    <div class="products-row">
      ...
      <div class="prod-card">
        ...
        <a href="{{ route('product.show', $product->slug) }}">
          <div class="prod-img-wrap">
            @if($product->thumbnail)<img src="{{ asset('storage/'.$product->thumbnail) }}" alt="{{ $product->name }}">
            @else<div class="prod-emoji">{{ $pIcon }}</div>@endif
          </div>
        </a>
        <div class="prod-body">
          <div class="prod-cat-tag">{{ $product->category->name ?? '' }}</div>
          <a href="{{ route('product.show',$product->slug) }}" style="text-decoration:none;color:inherit"><div class="prod-name">{{ $product->name }}</div></a>
          <div class="prod-weight">{{ $product->weight }} · {{ $product->unit }}</div>
          <div class="prod-footer">
            <div class="prod-price-wrap">
              <div class="prod-price">₹{{ number_format($product->current_price) }}</div>
            ...
            @if($product->is_in_stock)
            <form action="{{ route('cart.add') }}" method="POST">@csrf<input type="hidden" name="product_id" value="{{ $product->id }}"><input type="hidden" name="qty" value="1"><button type="submit" class="btn-add" onclick="addToCartAnim(this)">+</button></form>
            @endif
          </div>
        </div>
      </div>
...
<footer>
  <div class="footer-inner">
    ...
    <div class="footer-bottom">
      <div class="f-copy">© {{ date('Y') }} All You Want. All rights reserved. Made with ❤️</div>
...
<div class="mob-drawer" id="mobDrawer">
  ...
  <div class="mob-panel">
    ...
    <div style="display:flex;gap:10px;padding:16px 18px">
      @auth
        <a href="{{ route('account.index') }}" class="mob-btn-login">My Account</a>
        <form action="{{ route('logout') }}" method="POST" style="flex:1">@csrf<button type="submit" class="mob-btn-reg" style="width:100%">Logout</button></form>
      @else
        <a href="{{ route('login') }}" class="mob-btn-login">Login</a>
        <a href="{{ route('register') }}" class="mob-btn-reg">Register</a>
      @endauth
    </div>
    <div class="mob-nav-section">
      ...
      <a href="{{ route('category.show', $cat->slug) }}" class="mob-nav-link"><span class="mob-icon">{{ $icons[$cat->name] ?? '🛒' }}</span> {{ $cat->name }} <span class="mob-arrow">›</span></a>
      @endforeach
    </div>
    <div class="mob-nav-section" style="border-top:1px solid #f3f4f6">
      <div class="mob-nav-title">My Account</div>
      @auth
      <a href="{{ route('account.index') }}" class="mob-nav-link"><span class="mob-icon">👤</span> My Profile</a>
      <a href="{{ route('order.index') }}" class="mob-nav-link"><span class="mob-icon">📦</span> My Orders</a>
      <a href="{{ route('cart.index') }}" class="mob-nav-link"><span class="mob-icon">🛒</span> My Cart</a>
      @else
      <a href="{{ route('login') }}" class="mob-nav-link"><span class="mob-icon">🔐</span> Login</a>
      <a href="{{ route('register') }}" class="mob-nav-link"><span class="mob-icon">✅</span> Register Free</a>
      @endauth
    </div>
    <div class="mob-nav-section" style="border-top:1px solid #f3f4f6">
      <div class="mob-nav-title">Help</div>
      <a href="tel:9911011411" class="mob-nav-link"><span class="mob-icon">📞</span> 9911011411</a>
      <a href="{{ route('offers') }}" class="mob-nav-link"><span class="mob-icon">🔥</span> Today's Offers</a>
    </div>
  </div>
</div>
...
<div class="mob-bottom-nav">
  <div class="mob-bottom-nav-inner">
    <a href="{{ route('home') }}" class="mob-nav-item active">
      <svg viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
      Home
    </a>
    <a href="{{ route('shop') }}" class="mob-nav-item">
      <svg viewBox="0 0 24 24"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>
      Shop
    </a>
    <a href="{{ route('cart.index') }}" class="mob-nav-item" style="position:relative">
      <svg viewBox="0 0 24 24"><path d="M6 2L3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>
      Cart
      @if($cartCount > 0)<span class="mob-nav-badge">{{ $cartCount }}</span>@endif
    </a>
    <a href="{{ route('offers') }}" class="mob-nav-item">
      <svg viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
      Offers
    </a>
    <a href="{{ auth()->check() ? route('account.index') : route('login') }}" class="mob-nav-item">
      <svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
      Account
    </a>
  </div>
