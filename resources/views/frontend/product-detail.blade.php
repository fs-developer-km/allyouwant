...
<title>{{ $product->name }} — GroceryMart</title>
  ...
  <div class="nav-inner">
    ...
    <div class="nav-right">
      ...
      <a href="{{ route('cart.index') }}" class="btn-cart">
        ...
        @if($c>0)<span style="background:rgba(255,255,255,.25);border-radius:4px;padding:1px 6px;font-size:11px;font-weight:700">{{$c}}</span>@endif
      </a>
    </div>
  </div>
</nav>
...
<div class="breadcrumb">
  <div class="bc-inner">
    ...
    @endif
    <span style="color:#374151">{{ Str::limit($product->name, 40) }}</span>
  </div>
</div>
...
<div class="wrap">
  ...
  <div class="product-box">
    ...
    <div class="img-section">
      <div class="main-img" id="mainImg">
        ...
        @else
          <div class="main-img-emoji">{{ ['Vegetables'=>'🥬','Fruits'=>'🍎','Dairy & Eggs'=>'🥛','Meat & Fish'=>'🍗','Bakery'=>'🧁','Beverages'=>'🧃'][$product->category->name ?? ''] ?? '🛒' }}</div>
        @endif
      </div>
      @if($product->images->count() > 0)
      <div class="thumb-row">
        @if($product->thumbnail)
        <div class="thumb active" onclick="changeImg('{{ asset('storage/'.$product->thumbnail) }}', this)">
          <img src="{{ asset('storage/'.$product->thumbnail) }}" alt="Main">
        </div>
        @endif
        @foreach($product->images as $img)
        <div class="thumb" onclick="changeImg('{{ $img->image_url }}', this)">
    ...
    <div>
      <div class="prod-cat-tag">
        @if($product->category)<a href="{{ route('category.show', $product->category->slug) }}" style="color:var(--green)">{{ $product->category->name }}</a>@endif
        @if($product->is_bestseller) &nbsp;· 🔥 Bestseller @endif
        @if($product->is_new_arrival) &nbsp;· ✨ New Arrival @endif
      </div>
      <h1 class="prod-title">{{ $product->name }}</h1>

      <div class="prod-rating-row">
        @php $avgRating = $product->approvedReviews->avg('rating') ?: 0; $reviewCount = $product->approvedReviews->count(); @endphp
        <div class="stars">
          @for($i=1;$i<=5;$i++){{ $i <= round($avgRating) ? '★' : '☆' }}@endfor
        </div>
        <span class="rating-count">{{ number_format($avgRating,1) }} ({{ $reviewCount }} reviews)</span>
      ...
      <div class="price-section">
        <div style="display:flex;align-items:baseline;flex-wrap:wrap;gap:4px">
          <span class="price-current">₹{{ number_format($product->current_price) }}</span>
          @if($product->is_on_sale)
            <span class="price-old">₹{{ number_format($product->price) }}</span>
            <span class="discount-tag">{{ $product->discount_percent }}% OFF</span>
          @endif
        </div>
        <div class="price-per">Per {{ $product->unit }}{{ $product->weight ? ' · '.$product->weight : '' }}</div>
      </div>
      ...
      <form action="{{ route('cart.add') }}" method="POST" id="addCartForm">
        ...
        <div class="qty-section">
          ...
          <div class="qty-ctrl">
            ...
            <input type="number" name="qty" class="qty-val" value="1" min="1" max="{{ $product->stock_quantity }}" id="qtyInput">
            <button type="button" class="qty-btn" onclick="changeQty(1)">+</button>
          </div>
          <span style="font-size:12px;color:#9ca3af">{{ $product->stock_quantity }} available</span>
        </div>
      ...
      <div class="prod-meta">
        @if($product->sku)<div class="meta-row"><span class="meta-label">SKU:</span><span class="meta-val">{{ $product->sku }}</span></div>@endif
        @if($product->category)<div class="meta-row"><span class="meta-label">Category:</span><span class="meta-val"><a href="{{ route('category.show',$product->category->slug) }}" style="color:var(--green)">{{ $product->category->name }}</a></span></div>@endif
        <div class="meta-row"><span class="meta-label">Unit:</span><span class="meta-val">{{ $product->weight }} per {{ $product->unit }}</span></div>
  ...
  <div class="tabs-section">
    <div class="tabs">
      ...
      <button class="tab-btn" onclick="showTab('reviews', this)">⭐ Reviews ({{ $reviewCount }})</button>
    </div>

    <div class="tab-content active" id="tab-details">
      <table style="font-size:13.5px;border-collapse:collapse;width:100%;max-width:500px">
        @if($product->name)<tr><td style="padding:8px 16px 8px 0;color:#9ca3af;font-weight:500;width:140px">Product Name</td><td style="color:#374151;font-weight:500">{{ $product->name }}</td></tr>@endif
        @if($product->category)<tr><td style="padding:8px 16px 8px 0;color:#9ca3af;font-weight:500">Category</td><td style="color:#374151;font-weight:500">{{ $product->category->name }}</td></tr>@endif
        @if($product->weight)<tr><td style="padding:8px 16px 8px 0;color:#9ca3af;font-weight:500">Weight/Size</td><td style="color:#374151;font-weight:500">{{ $product->weight }}</td></tr>@endif
        <tr><td style="padding:8px 16px 8px 0;color:#9ca3af;font-weight:500">Unit</td><td style="color:#374151;font-weight:500">Per {{ $product->unit }}</td></tr>
        @if($product->sku)<tr><td style="padding:8px 16px 8px 0;color:#9ca3af;font-weight:500">SKU</td><td style="color:#374151;font-weight:500">{{ $product->sku }}</td></tr>@endif
        <tr><td style="padding:8px 16px 8px 0;color:#9ca3af;font-weight:500">Availability</td><td style="font-weight:600;color:{{ $product->is_in_stock ? '#16a34a' : '#dc2626' }}">{{ $product->is_in_stock ? '✅ In Stock ('.$product->stock_quantity.' units)' : '❌ Out of Stock' }}</td></tr>
    ...
    <div class="tab-content" id="tab-reviews">
      ...
      <div class="review-item">
        <div class="rev-top">
          <div class="rev-avatar">{{ substr($review->user->name ?? 'U', 0, 2) }}</div>
          <div><div class="rev-name">{{ $review->user->name ?? 'Customer' }}</div><div class="rev-date">{{ $review->created_at->format('d M Y') }}</div></div>
        </div>
        <div class="rev-stars">{{ str_repeat('★', $review->rating) }}{{ str_repeat('☆', 5-$review->rating) }}</div>
        @if($review->title)<div style="font-size:13.5px;font-weight:600;margin-bottom:4px">{{ $review->title }}</div>@endif
        <div class="rev-text">{{ $review->body }}</div>
      </div>
      @empty
      ...
      <div class="write-review">
        ...
        <form action="#" method="POST">
          ...
          <div class="star-select" id="starSelect">
            @for($i=1;$i<=5;$i++)<span data-val="{{ $i }}" onclick="setRating({{ $i }})">★</span>@endfor
          </div>
      ...
      <div style="text-align:center;padding:16px;background:#f8fafc;border-radius:8px;margin-top:16px;font-size:13.5px;color:#64748b">
        <a href="{{ route('login') }}" style="color:var(--green);font-weight:600">Login</a> to write a review
      </div>
      @endauth
    </div>
  </div>
  ...
  <div class="related-section">
    ...
    <div class="related-grid">
      ...
      <a href="{{ route('product.show', $rel->slug) }}" class="rel-card">
        <div class="rel-img">
          @if($rel->thumbnail)
            <img src="{{ asset('storage/'.$rel->thumbnail) }}" alt="{{ $rel->name }}">
          @else
            {{ ['Vegetables'=>'🥬','Fruits'=>'🍎','Dairy & Eggs'=>'🥛','Meat & Fish'=>'🍗','Bakery'=>'🧁','Beverages'=>'🧃'][$rel->category->name ?? ''] ?? '🛒' }}
          @endif
        </div>
        <div class="rel-body">
          <div class="rel-name">{{ $rel->name }}</div>
          <div class="rel-price">₹{{ number_format($rel->current_price) }}</div>
        </div>
      </a>
      @endforeach
    </div>
...
<div style="background:#0f1c14;color:rgba(255,255,255,.6);text-align:center;padding:20px;font-size:13px;margin-top:24px">
  © {{ date('Y') }} GroceryMart — <a href="{{ route('home') }}" style="color:#4ade80">Home</a> · <a href="{{ route('shop') }}" style="color:#4ade80">Shop</a>
</div>
