...
<div class="breadcrumb">
  <div class="bc-inner">
    <a href="{{ route('home') }}">All You Want</a>
    <span class="bc-sep">›</span>
    @if($product->category)
      <a href="{{ route('category.show',$product->category->slug) }}">{{ $product->category->name }}</a>
      <span class="bc-sep">›</span>
    @endif
    <span style="color:var(--text2)">{{ Str::limit($product->name, 60) }}</span>
  </div>
</div>
...
<div class="page-wrap">
  ...
  <div class="prod-layout">
    ...
    <div class="thumb-col">
      ...
      <div class="thumb-wrap active" onclick="changeImg('{{ asset('storage/'.$product->thumbnail) }}',this)">
        <img src="{{ asset('storage/'.$product->thumbnail) }}" alt="Main">
      </div>
      @endif
      @foreach($product->images as $img)
      <div class="thumb-wrap" onclick="changeImg('{{ asset('storage/'.$img->image) }}',this)">
        <img src="{{ asset('storage/'.$img->image) }}" alt="">
      </div>
      @endforeach
      @if(!$product->thumbnail && $product->images->count() === 0)
        @php $pIcon = ['Vegetables'=>'🥬','Fruits'=>'🍎','Dairy & Eggs'=>'🥛','Meat & Fish'=>'🍗','Bakery'=>'🧁','Beverages'=>'🧃','Instant Food'=>'🍜','Personal Care'=>'🧴','Household'=>'🏠','Pet Care'=>'🐾'][$product->category->name??'']??'🛒'; @endphp
        <div class="thumb-wrap active">{{ $pIcon }}</div>
      @endif
    </div>
    ...
    <div class="main-img-col">
      <div class="main-img-box">
        <div class="img-badges">
          @if(!$product->is_in_stock)<span class="img-badge ib-out">Out of Stock</span>
          @elseif($product->is_on_sale)<span class="img-badge ib-deal">{{ $product->discount_percent }}% off</span>@endif
          @if($product->is_new_arrival)<span class="img-badge ib-new">New</span>@endif
          @if($product->is_bestseller)<span class="img-badge ib-hot">Best Seller</span>@endif
        </div>
        @if($product->thumbnail)
          <img src="{{ asset('storage/'.$product->thumbnail) }}" alt="{{ $product->name }}" id="mainImgEl">
        @else
          @php $pIcon = ['Vegetables'=>'🥬','Fruits'=>'🍎','Dairy & Eggs'=>'🥛','Meat & Fish'=>'🍗','Bakery'=>'🧁','Beverages'=>'🧃','Instant Food'=>'🍜','Personal Care'=>'🧴','Household'=>'🏠','Pet Care'=>'🐾'][$product->category->name??'']??'🛒'; @endphp
          <div class="main-img-emoji">{{ $pIcon }}</div>
        @endif
      </div>
      <div class="main-img-actions">
        <button class="img-action-btn" id="wishBtn" onclick="toggleWishlist({{ $product->id }})">
    ...
    <div class="info-col">
      @if($product->category)
        <a href="{{ route('category.show',$product->category->slug) }}" class="prod-cat-link">
          {{ $product->category->name }}
        </a>
      @endif
      <h1 class="prod-h1">{{ $product->name }}</h1>
      <div class="prod-brand">
        Brand: <a href="{{ route('category.show',$product->category->slug ?? '#') }}" style="color:var(--blue2)">All You Want Grocery</a>
      ...
      <div class="rating-row">
        <div class="stars-display">
          ...
          <a href="#tab-reviews" onclick="openTab('reviews',document.querySelector('[data-tab=reviews]'))" class="star-val">{{ number_format($avgRating,1) }}</a>
        </div>
        <a href="#tab-reviews" class="review-cnt">{{ $reviewCount }} ratings</a>
      ...
      <div class="price-section" style="border-top:1px solid var(--border2);padding-top:14px">
        ...
        <div class="price-row">
          <div class="price-current">
            <span class="p-symbol">₹</span>{{ number_format($product->current_price) }}
          </div>
          @if($product->is_on_sale)
            <div style="font-size:14px;color:var(--green);font-weight:700;align-self:center">{{ $product->discount_percent }}% off</div>
          @endif
        </div>
        @if($product->is_on_sale)
          <div class="price-mrp-row">
            M.R.P.: <span class="price-mrp">₹{{ number_format($product->price) }}</span>
          </div>
          <div class="saving-text">You Save: ₹{{ number_format($product->price - $product->current_price) }} ({{ $product->discount_percent }}%)</div>
        @endif
        <div class="price-unit">Per {{ $product->unit }}{{ $product->weight ? ' · '.$product->weight : '' }}</div>
      ...
      <div class="stock-row">
        @if($product->is_in_stock)
          @if($product->stock_quantity <= 10)
            <span class="low-stock-txt">Only {{ $product->stock_quantity }} left in stock — order soon!</span>
          @else
      ...
      <div class="size-section">
        <div class="size-label">Size: <span>{{ $product->weight }}</span></div>
        <div class="size-pills">
          <div class="size-pill active">{{ $product->weight }}</div>
        </div>
      </div>
      @endif
      {{-- Highlights --}}
      <div class="highlights">
        ...
        @if($product->is_on_sale)
          <div class="highlight-item"><span class="highlight-icon">🏷️</span>On sale — {{ $product->discount_percent }}% discount applied</div>
        @endif
      </div>
      {{-- About Product --}}
      ...
      <div class="about-section">
        ...
        <ul class="about-list">
          @foreach(array_filter(array_map('trim', explode('.', $product->description))) as $line)
            @if(strlen($line) > 5)
              <li>{{ $line }}.</li>
            @endif
          @endforeach
      ...
      <div class="about-section">
        <div class="about-title">About this item</div>
        <ul class="about-list">
          <li>Fresh {{ $product->category->name ?? 'grocery' }} product — sourced daily</li>
          <li>Weight: {{ $product->weight ?? 'As per packaging' }} per {{ $product->unit }}</li>
    ...
    <div class="buy-box-col">
      <div class="buy-box">
        {{-- Price --}}
        <div class="buy-price">
          <span class="bp-sym">₹</span>{{ number_format($product->current_price) }}
        </div>
        @if($product->is_on_sale)
          <div class="buy-mrp-row">M.R.P.: <span style="text-decoration:line-through;color:var(--muted)">₹{{ number_format($product->price) }}</span></div>
          <div class="buy-saving">You save: ₹{{ number_format($product->price - $product->current_price) }}</div>
        @else
        ...
        @if($product->is_in_stock)
          @if($product->stock_quantity <= 10)
            <div class="buy-stock low">Only {{ $product->stock_quantity }} left!</div>
          @else
        ...
        <div class="buy-qty-row">
          ...
          <div class="qty-ctrl">
            ...
            <input type="number" class="qty-val" id="qtyInput" value="1" min="1" max="{{ $product->stock_quantity }}">
            <button type="button" class="qty-btn" onclick="changeQty(1)">+</button>
          </div>
        </div>
  ...
  <div class="info-tabs-section">
    <div class="tab-nav-bar">
      ...
      <button class="tab-nav-btn" onclick="openTab('reviews',this)" data-tab="reviews">Customer Reviews ({{ $reviewCount }})</button>
    ...
    <div class="tab-content-wrap">
      ...
      <div class="tab-pane active" id="tab-specs">
        <table class="spec-table">
          ...
          @if($product->weight)<tr><td>Net Weight</td><td>{{ $product->weight }}</td></tr>@endif
          <tr><td>Unit</td><td>Per {{ $product->unit }}</td></tr>
          @if($product->sku)<tr><td>Item model number</td><td>{{ $product->sku }}</td></tr>@endif
          <tr><td>Availability</td><td style="font-weight:700;color:{{ $product->is_in_stock?'var(--green)':'var(--red)' }}">{{ $product->is_in_stock ? 'In Stock ('.$product->stock_quantity.' units available)' : 'Out of Stock' }}</td></tr>
        ...
        <div style="margin-top:20px">
          ...
          <div style="font-size:14px;color:var(--text2);line-height:1.85">{{ $product->description }}</div>
        </div>
        @endif
      </div>
      ...
      <div class="tab-pane" id="tab-reviews">
        <div class="reviews-header">
          <div class="rating-overview">
            ...
            <div class="big-stars">@for($i=1;$i<=5;$i++){{ $i<=round($avgRating)?'★':'☆' }}@endfor</div>
            <div class="big-total">out of 5</div>
            <div style="font-size:13px;color:var(--muted);margin-top:4px">{{ $reviewCount }} global ratings</div>
          </div>
          <div class="rating-breakdown">
            @php $starCounts=[5=>0,4=>0,3=>0,2=>0,1=>0]; foreach($product->approvedReviews as $rev){ $starCounts[(int)$rev->rating]++; } @endphp
            @foreach([5,4,3,2,1] as $star)
            <div class="rb-row" onclick="document.querySelector('#tab-reviews .review-item')?.scrollIntoView({behavior:'smooth'})">
              <span class="rb-label">{{ $star }} star</span>
              <div class="rb-track"><div class="rb-fill" style="width:{{ $reviewCount>0?round($starCounts[$star]/$reviewCount*100):0 }}%"></div></div>
              <span class="rb-cnt">{{ $reviewCount>0?round($starCounts[$star]/$reviewCount*100):0 }}%</span>
            </div>
            @endforeach
          </div>
        </div>
        {{-- Review list --}}
        @php $ri_colors=['#007600','#7c3aed','#c45500','#007185','#cc0c39','#0d9488']; @endphp
        @forelse($product->approvedReviews as $i => $review)
        <div class="review-item">
          <div class="ri-head">
            <div class="ri-avatar" style="background:{{ $ri_colors[$i%6] }}">{{ strtoupper(substr($review->user->name??'U',0,2)) }}</div>
            <div><div class="ri-name">{{ $review->user->name ?? 'Customer' }}</div></div>
          </div>
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap">
            <span class="ri-stars">@for($s=1;$s<=5;$s++){{ $s<=$review->rating?'★':'☆' }}@endfor</span>
            @if($review->title)<span class="ri-title" style="font-size:14px;margin:0">{{ $review->title }}</span>@endif
          </div>
          <div class="ri-verified">✅ Verified Purchase &nbsp;|&nbsp; <span class="ri-date">{{ $review->created_at->format('d M Y') }}</span></div>
          <div class="ri-body">{{ $review->body }}</div>
          <div class="ri-helpful">
            <span>Was this review helpful?</span>
            <button class="helpful-btn" onclick="this.textContent='👍 Yes ('+((parseInt(this.textContent.match(/\d+/)?.[0])||0)+1)+')';this.disabled=true">👍 Yes ({{ rand(2,24) }})</button>
        ...
        <div class="wr-box">
          ...
          <form action="#" method="POST" onsubmit="submitReview(event)">
            ...
            <div class="wr-stars-row" id="wrStars">
              @for($i=1;$i<=5;$i++)
                <span class="wr-star active" onclick="setRating({{ $i }})">★</span>
              @endfor
            </div>
        ...
        <div style="margin-top:20px;padding:18px;background:#f9f9f9;border:1px solid var(--border2);border-radius:var(--r);text-align:center;font-size:14px;color:var(--text2)">
          <a href="{{ route('login') }}" style="color:var(--blue2);font-weight:700">Sign in</a> to write a customer review
        </div>
        @endauth
      </div>
      ...
      <div class="tab-pane" id="tab-qa">
        ...
        <div style="margin-top:16px;padding:14px;background:#f9f9f9;border:1px solid var(--border2);border-radius:var(--r);font-size:14px;color:var(--text2);text-align:center">
          <a href="{{ route('login') }}" style="color:var(--blue2);font-weight:700">Sign in</a> to ask a question
        </div>
        @endauth
      </div>
  ...
  <div class="related-section">
    ...
    <div class="prod-grid">
      ...
      @php $rIcon=['Vegetables'=>'🥬','Fruits'=>'🍎','Dairy & Eggs'=>'🥛','Meat & Fish'=>'🍗','Bakery'=>'🧁','Beverages'=>'🧃','Instant Food'=>'🍜','Personal Care'=>'🧴','Household'=>'🏠','Pet Care'=>'🐾'][$rel->category->name??'']??'🛒'; @endphp
      <div class="prod-grid-card">
        @if($rel->is_on_sale)<span class="pgc-badge pgcb-deal">{{ $rel->discount_percent }}% off</span>
        @elseif($rel->is_new_arrival)<span class="pgc-badge pgcb-new">New</span>@endif
        <a href="{{ route('product.show',$rel->slug) }}">
          <div class="pgc-img">
            @if($rel->thumbnail)<img src="{{ asset('storage/'.$rel->thumbnail) }}" alt="{{ $rel->name }}" loading="lazy">
            @else{{ $rIcon }}@endif
          </div>
        </a>
        <a href="{{ route('product.show',$rel->slug) }}" class="pgc-name">{{ $rel->name }}</a>
        <div class="pgc-stars">
          ★★★★☆
          <span>{{ rand(10,200) }}</span>
        </div>
        <div class="pgc-price-row">
          <div class="pgc-price"><span class="pcs">₹</span>{{ number_format($rel->current_price) }}</div>
          @if($rel->is_on_sale)
            <div class="pgc-price-old">₹{{ number_format($rel->price) }}</div>
            <div class="pgc-discount">{{ $rel->discount_percent }}% off</div>
          @endif
        </div>
        <div style="font-size:12px;color:var(--green);margin-bottom:8px">🚚 FREE Delivery</div>
        @if($rel->is_in_stock)
          <form action="{{ route('cart.add') }}" method="POST">
            @csrf<input type="hidden" name="product_id" value="{{ $rel->id }}"><input type="hidden" name="qty" value="1">
...
<div class="mob-sticky" id="mobSticky">
  <div class="mob-sticky-info">
    <div class="mob-sticky-name">{{ $product->name }}</div>
    <div class="mob-sticky-price">₹{{ number_format($product->current_price) }}</div>
  </div>
  @if($product->is_in_stock)
...
{{-- ═══ FLOATING BUTTONS ═══════════════════════ --}}
<a href="https://wa.me/919911011411?text=Hi! I want to know about {{ rawurlencode($product->name) }}" class="wa-btn" target="_blank" rel="noopener">💬</a>
...
function shareProduct(){
  if(navigator.share){
    navigator.share({title:'{{ addslashes($product->name) }}',text:'Check this out on All You Want Grocery!',url:window.location.href}).catch(()=>{});
  } else {
