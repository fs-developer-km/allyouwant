...
{{-- MOBILE-ONLY SUBCATEGORY STRIP --}}
@if($showSidebar)
<div class="cat-strip">
  <div class="cat-strip-inner">
    @foreach($sidebarItems as $item)
      <a href="{{ route('category.show', $item->slug) }}" class="cat-strip-item {{ $item->active ? 'active' : '' }}">
        ...
        <div class="cat-strip-title">{{ $item->name }}</div>
      </a>
    @endforeach
  </div>
</div>
...
<div class="page-wrap">
  <div class="main-layout {{ $showSidebar ? '' : 'no-sidebar' }}">
    ...
    {{-- SIDEBAR — always shown on desktop, list style --}}
    @if($showSidebar)
    <aside class="subcat-sidebar">
      <div class="subcat-sidebar-head">
        <div class="subcat-sidebar-title">{{ $sidebarRootCat->name }}</div>
      </div>
      <div class="subcat-list">
        @foreach($sidebarItems as $item)
          <a href="{{ route('category.show', $item->slug) }}" class="subcat-row {{ $item->active ? 'active' : '' }}">
            ...
            <div class="subcat-row-info">
              <div class="subcat-row-name">{{ $item->name }}</div>
              @if($item->count !== null)
                <div class="subcat-row-count">{{ $item->count }} items</div>
              @endif
            </div>
    ...
    <div class="prod-section">
      ...
      <div class="prod-topbar-compact">
        <div class="results-text">
          <strong>{{ $products->count() }}</strong> of <strong>{{ $products->total() }}</strong> products
          @if(request()->hasAny(['sort','filter','min_price','max_price']))
            <a href="{{ route('category.show', $category->slug) }}">Clear</a>
          @endif
        </div>
        <div class="compact-actions">
          <button class="sort-filter-toggle {{ $activeFilterCount > 0 || request('sort') ? 'has-active' : '' }}" id="sortFilterToggle" onclick="toggleSortPanel()">
            ⚙️ Sort & Filter
            <span class="badge">{{ $activeFilterCount + (request('sort') ? 1 : 0) }}</span>
          </button>
      ...
      <div class="sort-filter-panel" id="sortFilterPanel">
        ...
        <button class="filter-trigger-btn {{ $activeFilterCount > 0 ? 'has-active' : '' }}" onclick="openFilterDrawer()">
          🔧 Filters{{ $activeFilterCount > 0 ? " ($activeFilterCount)" : '' }}<span class="dot"></span>
        </button>
      </div>
...
<div class="filter-drawer" id="filterDrawer">
  ...
  <form method="GET" action="{{ route('category.show', $category->slug) }}" id="filterDrawerForm">
    <div class="filter-drawer-head">
      <h3>Filters</h3>
      <button type="button" class="filter-drawer-close" onclick="closeFilterDrawer()">✕</button>
    </div>

    @if(request()->hasAny(['filter','min_price','max_price']))
    ...
    <div class="filter-section">
      ...
      <div class="filter-opts">
        ...
        <label class="filter-opt">
          <input type="radio" name="filter" value="{{ $val }}" {{ request('filter')===$val ? 'checked' : '' }}>
          {{ $label }}
        </label>
        @endforeach
      </div>
    ...
    <div class="filter-drawer-foot">
      <a href="{{ route('category.show', $category->slug) }}" class="drawer-clear-btn">Clear All</a>
