...
<nav class="navbar">
  <div class="navbar-inner">
    ...
    <div class="nav-right">
      @auth
        <a href="{{ route('account.index') }}" class="nav-icon-btn"><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Account</a>
      @else
        <a href="{{ route('login') }}" class="nav-icon-btn"><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Login</a>
      @endauth
    ...
    <a href="{{ route('cart.index') }}" class="mob-cart-icon"><svg viewBox="0 0 24 24"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg></a>
...
<div class="cat-nav">
  <div class="cat-nav-inner">
    ...
    @foreach($allCategories ?? [] as $cat)
      ...
      <a href="{{ route('category.show', $cat->slug) }}" class="cat-link">{{ $icons[$cat->name] ?? '🛒' }} {{ $cat->name }}</a>
    @endforeach
    <a href="{{ route('offers') }}" class="cat-link active" style="color:#e53935;border-bottom-color:#e53935">🔥 Offers</a>
  </div>
</div>
...
<div class="page-wrap" style="padding-top:4px;padding-bottom:40px">
  ...
  <div class="offers-hero">
    ...
    <div class="oh-text">
      ...
      <div class="oh-sub">{{ $products->total() }} sale products available now. Grab them before they expire!</div>
    </div>
        ...
        <div class="prod-body">
          ...
          <div class="prod-footer">
            <div class="prod-price-wrap">
              ...
              <div class="prod-savings">Save ₹{{ number_format($product->price - $product->current_price) }}</div>
            </div>
  ...
  <div class="pagination-wrap">
    @if($products->onFirstPage())<span class="page-link disabled">‹</span>@else<a href="{{ $products->previousPageUrl() }}" class="page-link">‹</a>@endif
    @foreach($products->getUrlRange(max(1,$products->currentPage()-2),min($products->lastPage(),$products->currentPage()+2)) as $page => $url)
      <a href="{{ $url }}" class="page-link {{ $page==$products->currentPage()?'active':'' }}">{{ $page }}</a>
    @endforeach
    @if($products->hasMorePages())<a href="{{ $products->nextPageUrl() }}" class="page-link">›</a>@else<span class="page-link disabled">›</span>@endif
  </div>
  @endif
</div>
...
<div class="mob-drawer" id="mobDrawer">
  ...
  <div class="mob-panel">
    ...
    <div style="display:flex;gap:10px;padding:16px 18px">
      @auth<a href="{{ route('account.index') }}" style="flex:1;padding:11px;border:1.5px solid #0d6e39;border-radius:8px;font-size:13.5px;font-weight:600;color:#0d6e39;text-align:center;text-decoration:none">My Account</a>
      @else<a href="{{ route('login') }}" style="flex:1;padding:11px;border:1.5px solid #0d6e39;border-radius:8px;font-size:13.5px;font-weight:600;color:#0d6e39;text-align:center;text-decoration:none">Login</a><a href="{{ route('register') }}" style="flex:1;padding:11px;background:#0d6e39;border:none;border-radius:8px;font-size:13.5px;font-weight:600;color:#fff;text-align:center;text-decoration:none">Register</a>@endauth
    </div>
    <div class="mob-nav-title">Categories</div>
    @foreach($allCategories ?? [] as $cat)
      @php $icons = ['Vegetables'=>'🥬','Fruits'=>'🍎','Dairy & Eggs'=>'🥛','Meat & Fish'=>'🍗','Bakery'=>'🧁','Beverages'=>'🧃','Instant Food'=>'🍜','Personal Care'=>'🧴','Household'=>'🏠','Pet Care'=>'🐾']; @endphp
      <a href="{{ route('category.show', $cat->slug) }}" class="mob-nav-link">{{ $icons[$cat->name] ?? '🛒' }} {{ $cat->name }}</a>
    @endforeach
  </div>
</div>
...
<div class="mob-bottom-nav">
  <div class="mob-bottom-nav-inner">
    ...
    <a href="{{ route('offers') }}" class="mob-nav-item active" style="color:#0d6e39"><svg viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>Offers</a>
